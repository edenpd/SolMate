...
const MatchCard = ({
  ...
  const respondToMatch = (resp: String) => {
    ...
    axios
      .put(`${SERVER_ADDRESS}:${SERVER_PORT}/match`, uMatch)
      .then((res) => {
      ...
      artistsDOM.push(
        <View key={"artist" + i} style={appbarStyle.artist}>
          <Avatar.Image
            ...
            source={{ uri: user.Artists[i].images[0].url }}
          />
          {showNames && (
            <Text style={appbarStyle.artistName}>{user.Artists[i].name}</Text>
          )}
        </View>
      );
    }
      ...
      >
        <View style={appbarStyle.profileButton}>
          <IconButton
            ...
            color={Colors.white}
            size={35}
            onPress={() => onProfileClick(user._id)}
          />
        </View>
        <Card.Title style={appbarStyle.cardTitle} title={<Text></Text>} />
        <TouchableRipple
          borderless={true}
          style={appbarStyle.artistListRipple}
          underlayColor={"transparent"}
          onPress={onPressArtists}
        >
          <View style={appbarStyle.artistList}>{renderArtists()}</View>
        </TouchableRipple>
