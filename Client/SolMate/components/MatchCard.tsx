...
const MatchCard = ({ match, user }: MatchCardProps) => {
  ...
  const respondToMatch = (resp: String) => {
    ...
    axios
      .put("http://10.0.0.6:3001/match", uMatch)
      .then((res) => {
        // setMatches(res.data);
      ...
      >
        ...
        >
          <View style={appbarStyle.artistList}>
            <View style={appbarStyle.artist}>
              <Avatar.Image
                style={appbarStyle.artistImage}
                source={{
                  uri:
                    "https://assets.vogue.com/photos/5e40a77f89bdc90008ecc389/master/pass/billie-eilish-promo.jpg",
                }}
              />
              {showNames && (
                <Text style={appbarStyle.artistName}>Billie Eillish</Text>
              )}
            </View>
            <View style={appbarStyle.artist}>
              <Avatar.Image
                ...
                source={{
                  uri:
                    "https://i.guim.co.uk/img/media/c001a1444cc7dc29768ff55dfa283d0ea25651e1/0_312_3000_1800/master/3000.jpg?width=1200&height=900&quality=85&auto=format&fit=crop&s=f2f5d6e805577a64dd67472affba629a",
                }}
              />
              {showNames && (
                <Text style={appbarStyle.artistName}>Mac Miller</Text>
              )}
            </View>
            <View style={appbarStyle.artist}>
              <Avatar.Image
                ...
                source={{
                  uri:
                    "https://anothermanimg-dazedgroup.netdna-ssl.com/640/azure/anotherman-prod/360/6/366567.jpg",
                }}
              />
              {showNames && (
                <Text style={appbarStyle.artistName}>Arctic Monkeys</Text>
              )}
            </View>
          </View>
        </TouchableHighlight>
        <View style={appbarStyle.linearGradientContainer}>
          <Card.Content style={{ position: "absolute", bottom: 20, zIndex: 5 }}>
            <Text style={appbarStyle.cardTitle}>{`${user.firstName} ${
              user.lastName
            }, ${calcAge(user.birthday)}`}</Text>
            <Text
              style={{
                color: "white",
                fontSize: 15,
